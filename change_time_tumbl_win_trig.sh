...
jq --arg new_start_time "$new_start_time" '.resources |= map(if .properties.type == "TumblingWindowTrigger" then .properties.typeProperties.startTime = $new_start_time else . end)' "$json_file" > temp.json \
